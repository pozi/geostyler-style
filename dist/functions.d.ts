import { FunctionCall, Expression, PropertyType } from './style';
export type GeoStylerFunction = GeoStylerNumberFunction | GeoStylerStringFunction | GeoStylerBooleanFunction | GeoStylerUnknownFunction;
/**
 * An expression of a function that returns a number.
 */
export type GeoStylerNumberFunction = GeoStylerUnknownFunction | Fabs | Facos | Fadd | Fasin | Fatan | Fatan2 | Fceil | Fcos | Fdiv | Fexp | Ffloor | Finterpolate | Flog | Fmax | Fmin | Fmodulo | Fmul | Fpi | Fpow | Frandom | Frint | Fround | Fsin | Fsqrt | FstrIndexOf | FstrLastIndexOf | FstrLength | Fsub | Ftan | FtoDegrees | FtoNumber | FtoRadians;
/**
 * An expression of a function that returns a string.
 */
export type GeoStylerStringFunction = GeoStylerUnknownFunction | FnumberFormat | FstrAbbreviate | FstrCapitalize | FstrConcat | FstrDefaultIfBlank | FstrReplace | FstrStripAccents | FstrSubstring | FstrSubstringStart | FstrToLowerCase | FstrToUpperCase | FstrTrim | FtoString;
/**
 * An expression of a function that returns a boolean.
 */
export type GeoStylerBooleanFunction = GeoStylerUnknownFunction | Fall | Fany | Fbetween | Fdouble2bool | FequalTo | FgreaterThan | FgreaterThanOrEqualTo | Fin | FlessThan | FlessThanOrEqualTo | Fnot | FnotEqualTo | FparseBoolean | FstrEndsWith | FstrEqualsIgnoreCase | FstrMatches | FstrStartsWith;
